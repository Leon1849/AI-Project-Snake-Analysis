...
class GameController:
    ...
    def ai_play(self, algorithm):
        ...
        if isinstance(self.algo, GA):
            ...
            #Stuc poxac
            path = self.algo.run_algorithm(self.snake)
            self.update_path_finding_algo_1(path)
    ...
    #Nor sahmanac
    def update_path_finding_algo_1(self, result):
        if isinstance(result, Node):
        # If the result is a single Node, use it directly
            next_pos = result
        elif isinstance(result, list) and result:
            # If the result is a list of Nodes, take the first Node as the next position
            next_pos = result[0]
        else:
            # If result is None or an empty list, keep moving in the current direction
            next_pos = self.keep_moving()

        x = next_pos.x if isinstance(next_pos, Node) else next_pos[0]
        y = next_pos.y if isinstance(next_pos, Node) else next_pos[1]
